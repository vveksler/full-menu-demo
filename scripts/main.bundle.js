...
/***/ "./src/scripts/common/one-page-scroll.js":
/*!***********************************************!*\
  !*** ./src/scripts/common/one-page-scroll.js ***!
  \***********************************************/
/*! no static exports found */
...
"use strict";
eval("\n\nObject.defineProperty(exports, \"__esModule\", {\n  value: true\n});\nexports.default = onePageScroll;\nfunction onePageScroll(container) {\n  var sections = $(\".section\"); // все секции по id\n  var navItems = $(\".nav-fixed__item\");\n  var overlayMenuItems = $(\".nav-main__item\");\n\n  var duration = 1000;\n  var inScroll = false; // блокировка возможности проскролить во время анимации\n\n  //window.location.hash = \"\";\n\n  history.pushState(\"\", document.title, window.location.pathname); // При первой загрузке удалит hash чтобы избежать багов\n  setActiveItem(window.location.hash);\n\n  // Функция которая принимает hash и выполняет скролл до указаной секции\n  function scrollToSection(hash) {\n    if (hash) {\n      var activeSection = $(sections).filter(\".is-active\"); // текущяя активная секция\n      var newSectionId = $(\"section[id='\" + hash.substring(1) + \"'\"); // секция, в которую хотим перейти\n      var translateValue = $(newSectionId).index() - $(activeSection).index();\n\n      $(sections).removeClass(\"is-active\"); // У всех секций удаляет класс is-active\n      $(newSectionId).addClass(\"is-active\"); // Ставит кдасс is-active секции в которую перешли\n\n      $(container).css(\"transition\", \"transform \" + duration / 1000 + \"s ease-in-out\");\n      $(container).css(\"transform\", \"translateY(\" + -100 * translateValue + \"%)\");\n    }\n  }\n\n  function setActiveItem(hash, items) {\n    // выставление класса active в навигационом меню\n    if (window.location.hash) {\n      items.forEach(function (item) {\n        var links = $(item).children().attr(\"class\");\n\n        $(item).removeClass(\"active\");\n        $(\"a.\" + links + \"[href$='\" + hash + \"']\").parent().addClass(\"active\");\n      });\n    }\n  }\n\n  function handleDelay(hash) {\n    setTimeout(function () {\n      $(container).css(\"transition\", \"none\");\n      $(container).css(\"transform\", \"translateY(0)\");\n\n      window.location.hash = hash;\n      inScroll = false;\n    }, duration);\n  }\n\n  $(window).on(\"hashchange\", function (e) {\n    var hash = e.originalEvent.newURL.substring(e.originalEvent.newURL.indexOf(\"#\")); // Удаляем URL до решетки\n    var newSectionId = $(\"section[id='\" + hash.substring(1) + \"'\"); // находим секцию, в которую хотим перейти\n\n    $(sections).removeClass(\"is-active\"); // У всех секций удаляем класс is-active\n    $(newSectionId).addClass(\"is-active\"); // Ставим класс is-active секции в которую перешли\n\n    setActiveItem(hash, [navItems, overlayMenuItems]);\n  });\n\n  $(\"a\").on(\"click\", function (e) {\n    var hash = this.hash;\n    if (!this.hash) return;\n    e.preventDefault();\n\n    if (!inScroll) {\n      inScroll = true;\n      scrollToSection(hash);\n      handleDelay(hash);\n    }\n  });\n\n  $(\".wrapper\").on(\"wheel\", function (e) {\n    // Обработчик события по вращанию колесика мыши\n    if (!inScroll) {\n      inScroll = true;\n      var activeSection = $(sections).filter(\".is-active\"); // текущяя активная секция\n      var newSection = e.originalEvent.deltaY > 0 ? $(activeSection).next() : $(activeSection).prev();\n\n      if ($(newSection).length > 0) {\n        var hash = \"#\" + newSection.attr(\"id\");\n        scrollToSection(hash);\n        handleDelay(hash);\n      } else {\n        inScroll = false;\n      }\n    }\n  });\n}\n\n//# sourceURL=webpack:///./src/scripts/common/one-page-scroll.js?");
